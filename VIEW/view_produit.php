...
<?php
      ...
      if(isset($_GET['page'])) {
        ...
        $stmt = $pdo->prepare("SELECT * FROM article LIMIT ".ITEM_PER_PAGE." OFFSET $offset ");
        $stmt->execute();
        $rows = $stmt->fetchAll(PDO::FETCH_OBJ);

      }else {

        $stmt = $pdo->prepare("SELECT * FROM article LIMIT ".ITEM_PER_PAGE."");
        $stmt->execute();
        $rows = $stmt->fetchAll(PDO::FETCH_OBJ);
      ...
      if (isset($_GET['q']) AND !empty($_GET['q'])) {
        ...
        $stmt = $pdo->prepare("SELECT count(*) FROM article WHERE CONCAT(nom_article,categorie,quantite,prix_unitaire,date_expiration,date_fabrication) LIKE '%".$value."%'");
        $stmt->execute();
        $count = $stmt->fetch(PDO::FETCH_NUM)[0];
        ...
        if (isset($_GET['pages'])) {
          ...
          $offset = ITEM_PER_PAGE * ($current_page - 1);
          $stmt1 = $pdo->prepare("SELECT * FROM article WHERE CONCAT(nom_article,categorie,quantite,prix_unitaire,date_expiration,date_fabrication) LIKE '%".$value."%' LIMIT ".ITEM_PER_PAGE_SEARCH." OFFSET $offset");
          $stmt1->execute();
          $results = $stmt1->fetchAll(PDO::FETCH_OBJ);

        }else {
          
          $stmt = $pdo->prepare("SELECT * FROM article WHERE CONCAT(nom_article,categorie,quantite,prix_unitaire,date_expiration,date_fabrication) LIKE '%".$value."%' LIMIT ".ITEM_PER_PAGE_SEARCH."");
          $stmt->execute();
   ...
   <!--main container start-->
          <div style="margin: auto;"  class="main-container">
                <div style="padding:90px;" class="cards">
                    ...
                    <form action="view_produit.php" method="GET" >
                        <div class="d-flex justify-content-end">
                      ...
                      <table class="table table-striped" >
                              ...
                              <?php if (isset($results) AND !empty($results)):?>
                                ...
                                <tbody>
                                  <?php foreach ($results as $articles):?>
                                    <tr>
                                    <td><?=$articles->nom_article?></td>
                                    <td><?=$articles->categorie?></td>
                                    <td><?=number_format($articles->quantite ,0,',','.')?></td>
                                    <td><?=number_format($articles->prix_unitaire ,0,',','.')?></td>
