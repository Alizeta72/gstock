...
<?php
    ...
    if(isset($_GET['page'])) {
      ...
      $stmt = $pdo->prepare("SELECT * FROM users LIMIT ".ITEM_PER_PAGE." OFFSET $offset ");
      $stmt->execute();
      $rows = $stmt->fetchAll(PDO::FETCH_OBJ);

    }else {

      $stmt = $pdo->prepare("SELECT * FROM users LIMIT ".ITEM_PER_PAGE."");
      $stmt->execute();
      $rows = $stmt->fetchAll(PDO::FETCH_OBJ);
    ...
    if (isset($_GET['q']) AND !empty($_GET['q'])) {
      ...
      $stmt = $pdo->prepare("SELECT count(*) FROM users WHERE CONCAT(role,username,password,name,email) LIKE '%".$value."%'");
      $stmt->execute();
      $count = $stmt->fetch(PDO::FETCH_NUM)[0];
      ...
      if (isset($_GET['pages'])) {
        ...
        $offset = ITEM_PER_PAGE * ($current_page - 1);
        $stmt1 = $pdo->prepare("SELECT * FROM users WHERE CONCAT(role,username,password,name,email) LIKE '%".$value."%' LIMIT ".ITEM_PER_PAGE_SEARCH." OFFSET $offset");
        $stmt1->execute();
        $results = $stmt1->fetchAll(PDO::FETCH_OBJ);

      }else {
        
        $stmt = $pdo->prepare("SELECT * FROM users WHERE CONCAT(role,username,password,name,email) LIKE '%".$value."%' LIMIT ".ITEM_PER_PAGE_SEARCH."");
        $stmt->execute();
   ...
   <div style="margin: auto;"  class="main-container">
    <div style="padding:90px;" class="cards">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="d-flex justify-content-end">
                            <a href="../FORMULAIRE/form_register.php" class="btn btn-primary btn-sm me-3"><i class="fa-solid fa-plus"></i>
                                <span>Ajouter</span>
                            </a>
                        </div>
                    </div>
            ...
            <div class="dropdown-divider bordergreen" ></div>
                    ...
                    <form action="view_utilisateur.php" method="GET" >
                        <div class="d-flex justify-content-end">
                      ...
                      <table class="table table-striped" >
                              <thead style="background-color: #0a2558; color:#fff;">
                                ...
                                <?php foreach ($results as $user):?>
                                      ...
                                      <td><?=$user->email?></td>
                                      <td>
                                         <a href="../FORMULAIRE/modif_register.php?id=<?=$user->id?>"><i class="fa-regular fa-pen-to-square  btn-sm"  style="font-size:16pt"></i></a>
                                         <a onclick="return confirm('Etes vous sûr de vouloir supprimer cette ligne ? Cette action est irreversible!')" href="view_utilisateur.php?id=<?=$user->id?>"><i class="fa-solid fa-trash-can  btn-sm" style="color:red; font-size:16pt"></i></a>
                                      </td>
                                    </tr>
                                  <?php endforeach ?>
